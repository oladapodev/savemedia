...
export function SettingRow({ accessibilityLabel, control, description, icon = 'settings', onPress, title, value }: {
  ...
  const content = <Stack gap={control ? 'sm' : 0}
    style={{ borderBottomColor: colors.border, borderBottomWidth: 1, minHeight: 58, paddingVertical: 10 }}>
    <Inline gap="md">
    ...
    {control ? <View style={{ paddingLeft: 32, width: '100%' }}>{control}</View> : null}
  </Stack>;
