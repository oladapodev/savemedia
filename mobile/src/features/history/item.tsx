...
export function HistoryItem({ item, last = false, onOpen, onRetry, onShare, onToggle, selected = false, selecting = false }: {
  item: HistoryItemModel; onOpen?: (item: HistoryItemModel) => void; onRetry?: (item: HistoryItemModel) => void;
  last?: boolean; onShare?: (item: HistoryItemModel) => void; onToggle?: (item: HistoryItemModel) => void; selected?: boolean; selecting?: boolean;
}) {
  ...
  const body = <View style={{ borderBottomColor: selected ? colors.accent : colors.border, borderBottomWidth: last ? 0 : 1,
    ...
    <MediaThumbnail fallbackIcon={saved ? 'play' : 'warning'} label={`${item.title} thumbnail`}
      style={{ height: 58, width: 58 }} uri={item.thumbnailUrl} />
