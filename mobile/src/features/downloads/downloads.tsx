import { Button, EmptyState, Inline, PageHeader, ProgressBar, Screen, Stack, Surface, Text } from '../../ui';
import type { DownloadSelection } from '../../downloads/types';

export type ActiveDownloadItemModel = {
  id: string; title: string; platform: string; status: string; progress?: number;
...
export function DownloadsScreen({ items, notice, onCancel, onChoose }: {
  ...
  return <Screen scroll><Stack gap="lg">
    ...
    {items.length === 0 ? <EmptyState detail="Paste a link on Home to start a download." icon="download" title="No active downloads" />
      ...
      <Inline justify="between"><Stack gap="xs" grow><Text variant="label">{item.title}</Text>
